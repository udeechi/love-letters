...
export default function ImageOverlay({ images, isEditing, onSave }: ImageOverlayProps) {
  ...
  const imageElRefs = useRef<Map<number, HTMLElement>>(new Map());
  ...
  }, [images, onSave, selectedIndex]);

  const setImageEl = useCallback((index: number, el: HTMLElement | null) => {
    if (el) {
  ...
  useEffect(() => {
    ...
    moveableRef.current.on("drag", ({ target: tgt, left, top }) => {
      if (!tgt) return;
      const imgEl = tgt as HTMLElement;
    ...
    moveableRef.current.on("rotate", ({ target: tgt, rotate }) => {
      if (!tgt) return;
      (tgt as HTMLElement).style.transform = `rotate(${rotate}deg)`;
    });
      ...
      {images.map((img, index) => {
        ...
        if (img.type === "video") {
          return (
            <video
              key={img.public_id}
              ref={(el: HTMLElement | null) => setImageEl(index, el)}
              {...commonProps}
              autoPlay
              muted
              loop
