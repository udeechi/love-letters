...
export default function PageControls({
  ...
  if (isMobile) {
    return (
      <motion.div
        className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 px-4 py-2.5 bg-[#1a0d12]/85 border border-[#d4af37]/20 rounded-sm backdrop-blur-sm"
      ...
      >
        <button
          onClick={onPrev}
          disabled={atStart}
          className="w-10 h-10 flex items-center justify-center text-[#d4af37] hover:bg-[#d4af37]/10 rounded-sm disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
        >
        ...
        <div className="flex items-center gap-1.5 text-sm font-[family-name:var(--font-playfair)] text-[#d4af37]/70 min-w-[48px] justify-center">
          <span>{pageNum}</span>
          <span className="text-[#d4af37]/20">/ {totalPages}</span>
        </div>
        ...
        {isEditing && (
          <>
            <div className="w-px h-5 bg-[#d4af37]/15" />
            <button
              onClick={onAddPage}
              disabled={isSaving}
              className="px-3 py-1.5 text-sm text-[#d4af37]/60 hover:text-[#d4af37] hover:bg-[#d4af37]/10 rounded-sm disabled:opacity-50 transition-colors"
            >
              +
            </button>
            {totalPages > 1 && (
              <button
                ...
                disabled={isSaving}
                className="px-3 py-1.5 text-sm text-[#d4af37]/30 hover:text-[#a82d6a] hover:bg-[#a82d6a]/10 rounded-sm disabled:opacity-50 transition-colors"
              >
                Del
              </button>
            )}
